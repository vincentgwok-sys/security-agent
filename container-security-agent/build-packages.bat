...
powershell -Command "$lf=\"`n\"; $s='#!/usr/bin/env bash'+$lf+'set -euo pipefail'+$lf+'SCRIPT_DIR=\"$(cd \"$(dirname \"$0\")\" && pwd)\"'+$lf+''+$lf+'echo \"============================================\"'+$lf+'echo \" Container Security Agent (后端)\"'+$lf+'echo \"============================================\"'+$lf+''+$lf+'# 定位 Java — 优先级: JAVA_HOME > .java_home > PATH'+$lf+'find_java() {'+$lf+'  if [ -n \"${JAVA_HOME:-}\" ] && [ -x \"$JAVA_HOME/bin/java\" ]; then'+$lf+'    echo \"$JAVA_HOME/bin/java\"'+$lf+'    echo \"[INFO] 使用 JAVA_HOME: $JAVA_HOME\" >&2'+$lf+'    return 0'+$lf+'  fi'+$lf+'  if [ -f \"$SCRIPT_DIR/.java_home\" ]; then'+$lf+'    local h; h=$(head -1 \"$SCRIPT_DIR/.java_home\" | tr -d \"\r\" | xargs)'+$lf+'    if [ -n \"$h\" ] && [ -x \"$h/bin/java\" ]; then'+$lf+'      echo \"$h/bin/java\"'+$lf+'      echo \"[INFO] 使用 .java_home: $h\" >&2'+$lf+'      return 0'+$lf+'    fi'+$lf+'    echo \"[WARN] .java_home 存在但路径无效\" >&2'+$lf+'  fi'+$lf+'  local pj; pj=$(command -v java 2>/dev/null || true)'+$lf+'  if [ -n \"$pj\" ]; then'+$lf+'    echo \"$pj\"'+$lf+'    echo \"[INFO] 使用 PATH 中的 java: $pj\" >&2'+$lf+'    return 0'+$lf+'  fi'+$lf+'  return 1'+$lf+'}'+$lf+''+$lf+'JAVA_BIN=$(find_java) || {'+$lf+'  echo \"\"'+$lf+'  echo \"[ERROR] 未找到 Java (JDK 21+)\"'+$lf+'  echo \"\"'+$lf+'  echo \"请通过以下任一方式指定 Java 路径：\"'+$lf+'  echo \"  方式一（环境变量）：export JAVA_HOME=/path/to/jdk-21\"'+$lf+'  echo \"  方式二（本地文件）：echo /path/to/jdk-21 > .java_home\"'+$lf+'  echo \"\"'+$lf+'  echo \"当前 .java_home 文件位置: $SCRIPT_DIR/.java_home\"'+$lf+'  exit 1'+$lf+'}'+$lf+''+$lf+'mkdir -p \"$SCRIPT_DIR/logs\" \"$SCRIPT_DIR/reports\"'+$lf+''+$lf+'if [ ! -f \"$SCRIPT_DIR/application.yml\" ]; then'+$lf+'  echo \"[INFO] 首次运行，从 application-example.yml 复制配置模板...\"'+$lf+'  cp \"$SCRIPT_DIR/application-example.yml\" \"$SCRIPT_DIR/application.yml\"'+$lf+'  echo \"[INFO] >>> 请编辑 application.yml 填入你的 API Key，然后重新运行 start.sh <<<\"'+$lf+'  exit 0'+$lf+'fi'+$lf+''+$lf+'echo \"启动服务: http://localhost:8080\"'+$lf+'echo \"日志目录: $SCRIPT_DIR/logs/\"'+$lf+'echo \"报告目录: $SCRIPT_DIR/reports/\"'+$lf+'echo \"按 Ctrl+C 停止服务\"'+$lf+'echo \"\"'+$lf+''+$lf+'exec \"$JAVA_BIN\" -jar \"$SCRIPT_DIR/container-security-agent-%VERSION%.jar\"'; [System.IO.File]::WriteAllText('%BACKEND_DIR%\start.sh', $s)"
