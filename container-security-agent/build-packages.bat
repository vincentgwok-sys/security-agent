...
powershell -Command "$lf=\"`n\"; $s='#!/usr/bin/env bash'+$lf+'set -euo pipefail'+$lf+'SCRIPT_DIR=\"$(cd \"$(dirname \"$0\")\" && pwd)\"'+$lf+'BACKEND_DIR=\"$SCRIPT_DIR/container-security-agent-backend-%VERSION%\"'+$lf+'cd \"$BACKEND_DIR\"'+$lf+'exec ./start.sh \"$@\"'; [System.IO.File]::WriteAllText('release\run.sh', $s)"
